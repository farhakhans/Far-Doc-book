...
def extract_text_from_url(url):
    ...
    # Try to extract main content using common Docusaurus selectors
    ...
    # Look for content in main article, markdown content, or specific Docusaurus classes
    content_selectors = [
    ...
    # Clean up the text
    # Remove excessive whitespace and duplicate content
...
def ingest_book():
    ...
    # For testing, let's focus on the core documentation pages first
    # This will provide better results faster
    doc_urls = [url for url in urls if 'docs/' in url or 'intro' in url or 'overview' in url]

    print(f"Found {len(doc_urls)} documentation URLs to process")
    ...
    for i, url in enumerate(doc_urls):
        print(f"\nProcessing ({i+1}/{len(doc_urls)}): {url}")
